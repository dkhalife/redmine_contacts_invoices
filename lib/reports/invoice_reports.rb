      ...
      def classic_table(pdf, invoice)
        ...
        if InvoicesSettings.discount_after_tax?
          ...
          invoice.tax_groups_gst.each do |tax_group|
            lines << ['', '', '', '', "#{label_with_currency(:label_invoice_tax_gst, invoice.currency)} (#{invoice_number_format(tax_group[0])}%):", price_to_currency(tax_group[1], invoice.currency, :converted => false, :symbol => false)]
          end if invoice.tax_amount_gst> 0
          invoice.tax_groups_pst.each do |tax_group|
            lines << ['', '', '', '', "#{label_with_currency(:label_invoice_tax_pst, invoice.currency)} (#{invoice_number_format(tax_group[0])}%):", price_to_currency(tax_group[1], invoice.currency, :converted => false, :symbol => false)]
        ...
        pdf.bounding_box [(pdf.bounds.width / 2) + 20, pdf.cursor], :width => (pdf.bounds.width / 2) - 20  do
          pdf.font_size(11) do
            ...
            if InvoicesSettings.discount_after_tax?
              ...
              invoice.tax_groups_gst.each do |tax_group|
                invoice_total << ["#{label_with_currency(:label_invoice_tax_gst, invoice.currency)} (#{invoice_number_format(tax_group[0])}%):", price_to_currency(tax_group[1], invoice.currency, :converted => false, :symbol => false)]
              end if invoice.tax_amount_gst> 0
              invoice.tax_groups_pst.each do |tax_group|
                invoice_total << ["#{label_with_currency(:label_invoice_tax_pst, invoice.currency)} (#{invoice_number_format(tax_group[0])}%):", price_to_currency(tax_group[1], invoice.currency, :converted => false, :symbol => false)]
              end if invoice.tax_amount_pst> 0
